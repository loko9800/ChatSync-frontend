...
const mainFunction = ({ addBase, addComponents, config }) => {
  ...
  if (logs) {
    // Removed the console.log to prevent the "🌼 daisyUI 4.12.14" message
}
  ...
  if (logs) {
    ...
    if (themeInjector.themeOrder.length > 0) {
      // Removed the console.log to prevent the themes added message
    }
    
    if (themeInjector.themeOrder.length === 0) {
      // Removed the console.log to prevent the "All themes are disabled" message
    }
    
    const messages = []; // Keep it empty to avoid breaking the script
